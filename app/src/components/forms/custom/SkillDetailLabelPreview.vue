    ...
    <div class="rounded border border-gray-200 bg-white p-4 shadow-sm">
        <div class="flex items-start gap-3">
            ...
            <div class="min-w-0 flex-1">
                <span class="text-sm font-semibold text-gray-800">{{ t('SKILL_DETAIL_TAB_PREVIEW') }}</span>
                <p class="mt-1 text-xs leading-relaxed text-gray-500">
                    {{ t('SKILL_DETAIL_PREVIEW_DESCRIPTION') }}
                </p>
            </div>
        </div>
        ...
        >
            ...
            <p class="mt-2 max-w-sm text-center text-xs text-slate-500">
                {{ t('SKILL_DETAIL_PREVIEW_CAPTION') }}
            </p>
        </div>
    </div>
</template>
...
<script lang="ts" setup>
    ...
    const CATEGORY_LABEL_KEYS: Record<string, string> = {
        ...
        soft_skill: 'SKILL_CATEGORY_SOFT_SKILL',
        domain: 'SKILL_CATEGORY_DOMAIN',
        safety: 'SKILL_CATEGORY_SAFETY',
    };
    ...
    const displayText = computed(() => {
        ...
        return label || t('SKILL_DETAIL_PREVIEW_PLACEHOLDER');
    });
