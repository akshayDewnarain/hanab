        ...
        <template #header>
            <div class="flex justify-between">
                <div class="flex items-center gap-3">
                    <span class="font-semibold truncate">{{ t('GENERAL_DRINKS') }}</span>
                    <div class="bg-gray-300 px-2 rounded border border-gray-400 shadow-sm">
                        <span class="text-sm text-gray-700">{{ drinks.length + ` ${t('GENERAL_DRINKS')}` }} </span>
                    </div>
...
<script lang="ts" setup>
    ...
    const drinkColumns = new Drink()
        .columns()
        .filter((col: TableColumn) => ['name', 'price', 'unit', 'has_alcohol', 'active'].includes(col.name));
