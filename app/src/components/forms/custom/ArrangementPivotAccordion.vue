                ...
                <div class="flex items-center gap-3">
                    <div class="flex flex-col">
                        <div class="flex items-center gap-2">
                            <Icon class="w-4 h-4 text-[var(--color-primary)]" icon="material-symbols:restaurant-rounded" />
                            <span class="font-semibold truncate">{{ t(label) }}</span>
                        </div>
                        <span class="text-xs font-medium text-gray-500">Add or remove arrangements for this order.</span>
                    </div>
                    ...
                    >
                        <td class="py-2 px-3 text-center border-r border-gray-200">
                            <div class="flex justify-center">
                                <div
                                    v-if="getEntity(item.id)?.image?.thumb_url"
                                    class="rounded-full border border-gray-300 bg-gray-200 shadow-md"
                                >
                                    <img
                                        :src="getEntity(item.id)?.image?.thumb_url"
                                        alt=""
                        ...
                        <td class="py-2 px-3 text-gray-800 border-r border-gray-200 font-medium">
                            {{ getEntity(item.id)?.name ?? item.name ?? `#${item.id}` }}
                        </td>
                        <td class="py-2 px-3 text-center text-gray-600 border-r border-gray-200">
                            {{ formatPrice(getEntity(item.id)?.price) }}
                        </td>
...
<script lang="ts" setup>
    ...
    /** Shape of arrangement (or similar) rows used for display; maps store loose API records. */
    interface PivotEntityRow {
        name?: string;
        price?: number | string;
        drink_amount?: number | string;
        active?: boolean;
        image?: {
            thumb_url?: string;
            url?: string;
    ...
    const entityMap = ref<Map<number, Record<string, unknown>>>(new Map());
    ...
    watch(
        ...
        (list) => {
            ...
            for (const e of list ?? []) {
                const id = (e as Record<string, unknown>).id as number;
                if (id != null) m.set(id, e as Record<string, unknown>);
            }
            entityMap.value = m;
        },
        { immediate: true },
    ...
    function getEntity(id: number): PivotEntityRow | undefined {
        return entityMap.value.get(id) as PivotEntityRow | undefined;
    }
    ...
    function openModal(): void {
        ...
        modalService.open({
            ...
            onAccept(result: ModalResult | null) {
                ...
                const entities = (data.selectedEntities ?? []) as unknown as PivotEntityRow[];
